...
export default function Register() {
    ...
    return (
        <div className="border border-black/10 py-14 md:mt-20 mt-10 px-6 my-2 rounded-2xl shadow-lg max-w-3xl mx-4 sm:mx-auto justify-center items-center">
                ...
                <Formik className="flex flex-col justify-center" onSubmit={handleSubmit} initialValues={registerInitValues} validationSchema={registerValidation}>
                    {
                        ({values, errors, handleBlur, handleSubmit, handleChange, resetForm, touched, setFieldValue}) => (
                            <form onSubmit={handleSubmit} className="flex flex-col justify-center gap-6">
                                <div className="flex sm:flex-nowrap gap-5 flex-wrap items-center w-full">
                                ...
                                <Link className="text-sm text-green-500" onClick={() => resetForm()} to={"/login"}>Already have an account? Let&apos;s log in then</Link>
                            </form>
