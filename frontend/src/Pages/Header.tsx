...
export default function Header() {
    const {theme, user} = useSelector((store: RootState) => store.user)
    ...
    return (
        <header className="bg-black/90 w-full sticky p-2">
            <main className="max-w-7xl mx-auto flex justify-between items-center">
                <div className="text-white font-bold text-3xl flex items-center gap-6">
                    ...
                    <button onClick={() => navigate("/home")} className="flex items-center gap-4"><SiThunderbird className="text-xl" /><p>Birdy </p></button>
                ...
                <section className="md:hidden block text-base relative">
                    <input type="text" placeholder="Search" className="px-2 py-1 rounded-full bg-slate-200" />
                    <IoSearch className="absolute bottom-[6px] right-2 text-lg text-black" />
                </section>
            </main>
            ...
            <section className={`flex gap-10 items-center flex-col z-50 bg-slate-50 left-0 top-0 h-full w-full md:hidden transition duration-200 fixed ${modal ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                <div className="flex justify-between py-2 px-5 w-full items-center  text-xl">
                    <button onClick={() => navigate("/home")} className="font-bold italic flex  items-center gap-6"><SiThunderbird className="text-xl" /><p>Birdy </p></button>
